...
def mock_event(mock_device_entry: MagicMock):
    return Event(
        cast(
            EventType[EventDeviceRegistryUpdatedData | dict[str, Any]],
...
async def test_async_on_device_registry_updated(
    ...
    await mock_device_listener._async_on_device_registry_updated(
        cast(Event[dr.EventDeviceRegistryUpdatedData], mock_event)
    )
